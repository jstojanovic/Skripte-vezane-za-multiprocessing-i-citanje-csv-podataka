...
def read_data(start_date):
    ...
    return dictionari
def get_same_or_newer(start_date, dictionari):    
    ...
    for row in dictionari.items(): 
        ...
        # employees at the minimal date.
        if row_date == min_date:
            min_date_employees.append("{}".format(row[0]))

    return min_date, min_date_employees

def list_newer(start_date):
    dictionari = read_data(start_date)
    while start_date < datetime.datetime.today():
        start_date, employees = get_same_or_newer(start_date, dictionari)
